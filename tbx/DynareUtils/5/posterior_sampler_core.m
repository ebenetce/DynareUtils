...
if whoiam
    % initialize persistent variables in C()
...
InitialSeeds = record.InitialSeeds;
algo = options_.DynareRandomStreams.algo;
seed = options_.DynareRandomStreams.seed;

% Create waitbars and queue
q = parallel.pool.DataQueue;
afterEach(q, @UpdateBar)

% Deal with many waitbars issue + 10 markov chains
multiWaitbar('CloseAll');
cleanupObj = onCleanup(@() multiWaitbar('CloseAll'));
for curr_block = fblck:nblck

    bar_title = sampler_options.parallel_bar_title;
    Label = [bar_title ' (' int2str(curr_block) '/' int2str(options_.mh_nblck) ')...'];
    multiWaitbar(['Chain', curr_block], 0, 'Color', 'b', 'Relabel', Label);
end

% Initialize storage variables
draw_index_current_file_all = zeros(1, numel(fblck:nblck) );
LastParameters = record.LastParameters;
LastLogPost = record.LastLogPost;
AcceptanceRatio = record.AcceptanceRatio;
FunctionEvalPerIteration = record.FunctionEvalPerIteration;
OutputFileName = cell(1, numel(fblck:nblck));
LastSeedsR = [];
...
parfor curr_block = 1:nblck
    ...
    try
        ...
        set_dynare_seed(algo, seed);
   
        % Set the state of the RNG
        set_dynare_random_generator_state(InitialSeeds(curr_block).Unifor, InitialSeeds(curr_block).Normal);
    catch
        disp('catch')
        seed
        % If the state set by master is incompatible with the slave, we only reseed
        set_dynare_seed(seed+curr_block);
    end
    ...
    while draw_iter <= nruns(curr_block)
        ...
        if mod(draw_iter, refresh_rate)==0
            if accepted_draws_this_chain/draw_iter==1 && sum(neval)>1
                send(q, struct('Name', ['Chain', curr_block], 'Text', [bar_title ' (' int2str(curr_block) '/' int2str(options_.mh_nblck) ')...'], 'Value', prtfrc))
            else
                send(q, struct('Name', ['Chain', curr_block], 'Text', [bar_title ' (' int2str(curr_block) '/' int2str(options_.mh_nblck) ') ' sprintf('Current acceptance ratio %4.3f', accepted_draws_this_chain/draw_iter)], 'Value', prtfrc))
            end
        ...
        if (draw_index_current_file == InitSizeArray(curr_block)) || (draw_iter == nruns(curr_block)) % Now I save the simulations, either because the current file is full or the chain is done
            ...
            toSave = struct('x2',x2,'logpo2',logpo2,'LastSeeds',LastSeeds,'accepted_draws_this_chain',accepted_draws_this_chain,'accepted_draws_this_file',accepted_draws_this_file,'feval_this_chain',feval_this_chain,'feval_this_file',feval_this_file);
            save([BaseName '_mh' int2str(NewFile(curr_block)) '_blck' int2str(curr_block) '.mat'],'-fromstruct',toSave);
            ...
            if draw_iter == nruns(curr_block) % I record the last draw...
                LastParameters(curr_block,:) = x2(end,:);
                LastLogPost(curr_block) = logpo2(end);
            end
    ...
    if nruns(curr_block)
        AcceptanceRatio(curr_block) = accepted_draws_this_chain/(draw_iter-1)
        FunctionEvalPerIteration(curr_block) = feval_this_chain/(draw_iter-1);
        [LastSeedsR(curr_block).Unifor, LastSeedsR(curr_block).Normal] = get_dynare_random_generator_state();
    end
    OutputFileName{curr_block} = {[MetropolisFolder,filesep], [ModelName '_mh*_blck' int2str(curr_block) '.mat']};
    draw_index_current_file_all(curr_block) = draw_index_current_file;
end % End of the loop over the mh-blocks.

record.LastParameters = LastParameters;
record.LastLogPost = LastLogPost;
record.AcceptanceRatio = AcceptanceRatio;
record.FunctionEvalPerIteration = FunctionEvalPerIteration;
record.LastSeeds = LastSeedsR;

myoutput.record = record;
myoutput.irun = draw_index_current_file_all(end);
...
function UpdateBar(task)
    multiWaitbar(task.Name, task.Value, 'Relabel', task.Text);
end